...
include { MANTA_HS } from './modules/manta.nf'
...
include { JASMINE_SV_MERGE } from './modules/jasmine_sv_merge.nf'
include { FILTER_JASMINE_SVMERGE } from './modules/filter_jasmine_sv_merge.nf'
...
workflow {
    ...
    if (!params.cnv_model) { error "Error: Please specify cnv model with --cnv_model" }
    ...
    if (!params.pangenome_data_dir) { 
        error "Error: Please specify path to pangenome input files using --pangenome_data_dir" 
    }
    if (!params.pangenome_data_prefix) { 
        error "Error: Please specify pangenome input files prefix using --pangenome_data_prefix"
    }
    if (!params.biallelic_vcf) { 
        error "Error: Biallelic Pangenome VCF (--biallelic_vcf) is required."
    }
    if (!params.target_bed) { 
        error "Error: Bed file of protein coding regions (--target_bed) is required."
    }
    ...
    // Generate metrics
    metrics_out = METRICS(bwa_mem_out.sorted_bam, reference_fasta_ch, target_bed_ch, dest_prefix_ch)
    ...
    // Call unknown SVs with Manta, Delly, PanGenie, and Jasmine 
    manta_out = MANTA_HS(reference_fasta_ch, dedup_out.deduped_bam)
    delly_out = DELLY_SV(reference_fasta_ch, dedup_out.deduped_bam)
    jasmine_out = JASMINE_SV_MERGE(reference_fasta_ch, pangenie_processing_out.pangenie_input_jasmine, manta_out.manta_vcf, delly_out.delly_vcf)
    
    header_file_ch = Channel.value(file('resources/missing_header_lines.txt'))

    combined_sv_calls = FILTER_JASMINE_SVMERGE(jasmine_out.jasmine_out_vcf, pangenie_processing_out.pangenie_input_jasmine, header_file_ch)

    // Combine SNV/INDEL VCF and SV VCF for STR/VNTR annotation
    concat_sv_smallvar = CONCAT1(combined_sv_calls.combined_svs, dnascope_out.dnascope_vcf)
